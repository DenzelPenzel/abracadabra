...
#[test]
fn and_has_an_independently_pinned_wire_encoding() {
    let program = Program::new(0, vec![Instruction::And(Width::Word), Instruction::Ret]);
    let expected = [
        0x56, 0x4d, 0x50, 0x42, 0x01, 0x00, 0x10, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x23, 0x02, 0x01,
    ];

    assert_eq!(encode(&program).expect("and must encode"), expected);
    assert_eq!(
        decode(&expected).expect("physical and must decode"),
        program
    );
}

#[test]
fn or_has_an_independently_pinned_wire_encoding() {
    let program = Program::new(0, vec![Instruction::Or(Width::Word), Instruction::Ret]);
    let expected = [
        0x56, 0x4d, 0x50, 0x42, 0x01, 0x00, 0x10, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x24, 0x02, 0x01,
    ];

    assert_eq!(encode(&program).expect("or must encode"), expected);
    assert_eq!(decode(&expected).expect("physical or must decode"), program);
}

#[test]
fn shl_has_an_independently_pinned_wire_encoding() {
    let program = Program::new(0, vec![Instruction::Shl(Width::Word), Instruction::Ret]);
    let expected = [
        0x56, 0x4d, 0x50, 0x42, 0x01, 0x00, 0x10, 0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x25, 0x02, 0x01,
    ];

    assert_eq!(encode(&program).expect("shl must encode"), expected);
    assert_eq!(
        decode(&expected).expect("physical shl must decode"),
        program
    );
}
...
fn every_typed_width_register_and_condition_round_trips() {
    ...
    for width in widths {
        ...
        instructions.push(Instruction::And(width));
        instructions.push(Instruction::Or(width));
        instructions.push(Instruction::Shl(width));
...
mod v2 {
    use vmp_vm::bytecode::{
        self, Condition, Register, Width, MAX_CONTAINER_SIZE, MAX_INSTRUCTIONS,
    };
    ...
    fn literal_wire_all_opcodes_and_version_separation() {
        let literal = [
            86, 77, 80, 66, 2, 0, 16, 0, 24, 0, 0, 0, 0, 0, 0, 0, 0x10, 1, 0xa5, 0x11, 2, 15, 0x12,
            4, 0, 0x13, 8, 0x14, 0x30, 23, 0, 0, 0, 0x31, 15, 23, 0, 0, 0, 1,
        ];
    ...
    fn truncation_every_container_and_instruction_prefix() {
        let code = [
            0x10, 8, 1, 2, 3, 4, 5, 6, 7, 8, 0x11, 8, 0, 0x12, 8, 1, 0x13, 8, 0x14, 0x30, 0, 0, 0,
            0, 0x31, 0, 0, 0, 0, 0, 1,
        ];
